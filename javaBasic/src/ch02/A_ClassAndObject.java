...
class ExampleClass1 {
	...
	void method4 () {
		...
		System.out.println(staticAttribute);		//인스턴스 메서드에서는 인스턴스변수,메서드 스태틱변수,메서드 사용가능
	...
	// 클래스와 같은 이름을 가지고 있고, 반환타입이 존재하지 않음(무조건 인스턴스가 반환됨)
	// 클래스명 ([매개변수, ...]) { 인스턴스가 생성될떄 할 작업 }
	ExampleClass1(int attribute1, double attribute2) {					// 외부에서 자료를 가져와야할때 매개변수를 지정
		System.out.println("ExampleClass1 인스턴스 생성!!");
		// 의존성이 내부에서 주입하는 형태
		// 속성값이 생성 이전에 결정이 되었기 때문에 코드의 안정성이 저하
		//attribute1 = 99;
		//attribute2 = -99.99;
		
		// 의존성을 외부에서 주입하는 형태
		// 속성값이 생성 당시에 결정되기 때문에 코드의 안정성을 향상
		// attribute1 = attribute1;			    // 매개변수 att~안에 매개변수 att~를 넣음
		this.attribute1 = attribute1;			//this - class에서 생성된 인스턴스변수를 가르킴
	...
	// 오버로딩 조건
	// 1. 메서드 이름을 중복해서 작성하지만 매개변수의 타입 및 개수를 작성
...
public class A_ClassAndObject {
	...
	public static void main(String[] args) {                      // 기능 method : main
		// 지역 변수
		//String team;       
		
		// 인스턴스: 특정 클래스로 정의된 것을 실체화 한것 (객체)
		// 클래스명 참조변수명 = new 클래스명();
		ExampleClass1 instance1 = new ExampleClass1(0,0);			// (객체)인스턴스 생성 (ExampleClass1 클래스의 속성과 메소드를 가지고있음)
		...
		System.out.println(instance2.attribute2);			// instance1 과 2는 독립적
		
		// 인스턴스가 가지고 있는 메서드 호출 방법
		// 인스턴스.메서드();
		instance1.method1(1, 2);		// 매개변수가 있을 경우 (int arg1, int arg2) 데이터를 표현해줘야됨
		instance1.method2();
		instance1.method3(1);
		instance1.method4();
		...
		//ExampleClass1.attribute1 = 10;		에러
		//ExampleClass1.method1(0, 0);			에러
		instance1.staticAttribute = 99;						// 스태틱변수 인스턴스로 접근 > 클래스로 접근을 권장			
		System.out.println(instance1.staticAttribute);
		System.out.println(instance2.staticAttribute);		// instance1 만 변경해도 2에적용됨
