...
public class B_Datatype {
	...
	public static void main(String[] args) {
		...
		long long1 = 2_147_483_648L;   //끝에다 L을붙임
		...
		// 소수점 6-7자리에서 오차가 발생
		float float1 = 3.1415926535F;     // 끝에 F입력
		...
		System.out.println(byteNumber);             // 데이터손실
		
		short shortNumber = 97;
		char character = 97;
		//shortNumber = character;                 // 데이터 형태가 다름
		
		int intNumber = 100;
		float floatNumber = 100;
		//intNumber = floatNumber;                 // 대체로 자동변환이 되는 경우라도 명시적으로 변환함
		...
		// 데이터타입[] 변수명(배열명);
		// 생성
		// 변수명 = new 데이터타입[크기];                 new를 사용하면 전부 주소가 들어감(참조형 데이터 타입)
		int[] numbers;
		numbers = new int[5];
		...
		item = numbers[0];
		System.out.println(item);                       // 생성하고 초기화를 하지않음 -> 0
		numbers[0] = 99;
		System.out.println(item);
		item = numbers[0];                              // 재할당
		System.out.println(item);    
		
		// 인덱스 범위를 초과하는 인덱스에 접근할 시 예외발생(컴파일은 가능)
		// System.out.println(numbers[5]);                
		
		// numbers : {99, 0, 0, 0, 0}
		numbers2 = numbers;
		
		// numbers2 : {99, 0, 0, 0, 0}   실제값이 아니라 주소가 들어가 있음
		numbers[4] = -99;
		// numbers : {99, 0, 0, 0, -99}
		// numbers2 : {99, 0, 0, 0, -99}     // numbers2의 4번째 값도 -99가 됨
		
//		System.out.println(numbers2[4]);     // -99가나옴,  numbers와 같은 주소값이기 때문에(실제값이 아니라 주소를 보기때문)
...
//		System.out.println(number2);        // 0
		...
		// 문자열을 다루는데 유용한 여라가지 기능을 제공해주고 있음
		...
		System.out.println(statement.equals("My name is hulk"));      // true or false가 나옴
		System.out.println(statement.compareTo("Apple"));             // 사전순으로 먼저이면 양수 나중이면 음수가 나옴 소문자일경우 음수
		...
		System.out.println(statement.indexOf("하마"));       // 존재하지 않을때는 -1
		System.out.println(statement.lastIndexOf("그림"));
		System.out.println(statement.lastIndexOf("하마"));   // 존재하지 않을때는 -1
		...
		// null : 아직 어떠한 것도 지정되지 않은 상태           참조형 타입에서만 사용
		String string2 = null;
		//System.out.println(string2.substring(48));         // NullPointerException null은 주소값도 가지지 않음
