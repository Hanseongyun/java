...
// Test3 형태로 클래스를 사용하게되면 타입의 안정성이 떨어짐
class Test3 {
	String subject;
	Object score;
}
...
class Test4<T>{								//T: 매개타입
	String subject;
	T score;								// 클래스에서 변수의 속성을 나중에 결정하고싶을때
}

class Test5 {
	
	// 제너릭 매서드 : 해당 메서드의 매개변수의 타입이나 반환 타입에서 사용할 타입을 외부에서 받아오려 할 때 사용
	// <매개타입> 반환타입 매서드명 (<매개타입> 매개변수) {...}
	static <T> void method(List<T> arg) {				// 제너릭은 업캐스팅이 안됨
		System.out.println(arg);
	}
	// ?: 와일드 카드로 제너릭에 대해서 모든 타입을 받을 수 있도록 함
	static void wildCardMethod (List<?> arg) {			// ? -> 코드의 안정성이 떨어짐
		System.out.println(arg);
	}
	// ? extends 클래스명: 특정 클래스 혹은 특정 클래스를 확장한 클래스만 제너릭으로 받을 수 있도록 제한함
	// (본인클래스 또는 자손클래스)
	static void extendMethod (List<? extends MyClass> arg) {		// MyClass 이거나 MyClass를 확장한 녀석만 받아짐
		System.out.println(arg);
	}
...
//		Test5.method(10);
		...
		Test5.method(arrayList1);			// method를 호출하는 순간에 String
		Test5.method(arrayList2);			// method를 호출하는 순간에 Boolean
