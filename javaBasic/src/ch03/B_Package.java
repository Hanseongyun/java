...
//		arrayList.add("aaaa");
		...
		// Set: 데이터를 저장할 때 중복을 허용하지 않고 순서가 존재하지 않음(집합)			순서가 없다 -> 인덱스로 접근할 수 없다.
		Set<String> set;
...
//		set = new HashSet<>();
		...
		// 제거하고자 하는 키가 없더라도 예외가 발생하지 않음
//		map.remove("Apple");
//		System.out.println(map);
		// remove 작업에 성공하면 해당 key의 값을 반환
		...
		Map<String, Object> human2 = new HashMap<>();	// 클래스로 처리할 수 있지만 귀찮아서? Map으로 처리하는 경우
		human2.put("name", "홍길동");
		human2.put("address", "부산광역시 진구");
		human2.put("age", "열살");					// Object를 사용하면 10 대신 "열살" 도 가능
